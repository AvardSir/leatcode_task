class Solution {
    ...
    findCheapestPrice(n, flights, src, dst, k) {
        ...
        minH.push([src, 0, 0])
        while (minH.size()) {
            let [curDtns, CurCost, curK] = minH.pop()
            if (curDtns == dst) {
                minCost = Math.min(minCost, CurCost)
                continue
            }

            if (!fromTo.has(curDtns)) {
                continue
            }
            if (curK == k) {
                continue
            }
            if (visitSet.has(curDtns)) {
                continue
            }

            visitSet.add(curDtns)
            let curArChildre = fromTo.get(curDtns)
            for (let i = 0; i < curArChildre.length; i++) {
                const element = curArChildre[i];
                // to,cost
                let to = element[0]
                let cost = element[1]

                let nextNode = [to, cost + CurCost, curK + 1]
                minH.push(nextNode)
                // [dstn,cost,curK]
            }
        }
