class Solution {
    ...
    swimInWater(grid) {
        ...
        let isFind = false
        function adIJ(i, j, curMinHGridVal) {
            ...
            // grid[i][j] = Infinity
            minH.push([[i, j], curMinHGridVal > curGridVal ? curMinHGridVal : curGridVal])
        }
        ...
        while (minH.size()) {
            ...
            if (i == endJ && j == endJ) {
                return curMinHGridVal
            }
            grid[i][j] = Infinity
            adIJ(i + 1, j, curMinHGridVal)
            adIJ(i, j + 1, curMinHGridVal)
            adIJ(i - 1, j, curMinHGridVal)
            adIJ(i, j - 1, curMinHGridVal)
        }
