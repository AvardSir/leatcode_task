...
class Solution {
    ...
    reverseKGroup(head, k) {
        ...
        while (true) {
            ...
            let lastNodeSeq = curNode // это конец=3
            // startCur это начало =1
            // чтоб работало надо 
            // обрубить конец

            let StartNextNode
            if (lastNodeSeq.next) {
                StartNextNode = lastNodeSeq.next
            }
            else {
                break
            }
            ...
            let reveNodeListSeq = reversNode(startCur)
            3
            // это получается надо пристыковать к началу
